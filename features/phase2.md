...
| C Convex backend | in progress | `../plantry-stream-C` on `feat/C-swap` | Slices 1-3 shipped. Slice 4 (`getSlotAlternatives` query + `swapDish` mutation in `app/convex/swap.ts`) spawned 2026-06-09 in parallel with Stream D slice 3. `finalizeWeek`, auto-recovery middleware, `getGroceryList` queued. |
| D Frontend | in progress | `../plantry-stream-D` on `feat/D-swap-ui` | Slices 1 and 2 shipped. Slice 3 (segmented Swap/Custom mode in SlotEditor, alternatives subscribed via `getSlotAlternatives`, swap via `swapDish` with tagged-union version-mismatch + dish-not-eligible handling) spawned 2026-06-09 in parallel with Stream C slice 4. Grocery list rendering queued next. |
