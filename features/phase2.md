...
| B Engine | in progress | `../plantry-stream-B` on `feat/B-composition` | Slices 1 (eligibility) and 2 (schedule) shipped. Slice 3 (composition per §3 with §3.1 and §3.2) spawned 2026-06-08. Priority (§4), cap (§5), consolidation (§6), `generateWeek`, `rankCandidatesForSlot`, simulation harness queued. |
...
| D Frontend | in progress | `../plantry-stream-D` on `feat/D-shell-and-readonly-week` | Slice 1 (app shell, passcode gate, identity picker, read-only current-week view, offline last-known cache) spawned 2026-06-08. Swap UI, custom one-offs, comments, grocery list queued for later slices. |
