...
| B Engine | in progress | `../plantry-stream-A` on `feat/B-generateweek` | Slices 1-6 shipped (eligibility, schedule, composition, priority, consolidation, cap). Slice 7 (`generateWeek` + `rankCandidatesForSlot` composing the pipeline) in flight; will rebase onto post-slice-6 main and import the real `applyCap`. Simulation harness queued after. |
