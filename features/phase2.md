...
| B Engine | in progress | `../plantry-stream-B` on `feat/B-consolidation` | Slices 1-4 shipped. Slice 5 (consolidation §6 + wires priority step 3 to a real implementation) spawned 2026-06-09. Cap (§5), `generateWeek`, `rankCandidatesForSlot`, simulation harness queued. |
...
| D Frontend | in progress | `../plantry-stream-D` on `feat/D-write-affordances` | Slice 1 shipped in PR #13. Slice 2 (identity Convex sync via `setUserProfile`, inline custom one-off entry, comment-on-dish + comment-on-day affordances) spawned 2026-06-09. Swap UI waits for Stream B `rankCandidatesForSlot` + a C `swapDish` mutation. Grocery list waits for B §6 + C `getGroceryList`. |
